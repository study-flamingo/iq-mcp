...
class Observation(BaseModel):
    ...
    content: str = Field(..., title="Observation content", description="The observation content", alias="contents")
    durability: DurabilityType = Field(..., title="Durability", description="How long this observation is expected to remain relevant")
    timestamp: str | None = Field(..., title="Timestamp", description="ISO date string when the observation was created", alias="ts")
...
class Entity(BaseModel):
    ...
    name: str = Field(..., title="Entity name", description="Unique identifier for the entity", alias="entity_name")
    entity_type: str = Field(..., title="Entity type", description="Type classification (e.g., 'person', 'organization', 'event')", alias="entityType")
    observations: list[Observation] = Field(default_factory=list, title="List of observations", description="Associated observations with content, durabiltiy, and timestamp", alias="observation")
...
class Relation(BaseModel):
    ...
    from_entity: str = Field(..., title="From entity", description="Source entity name", alias="from")
    to_entity: str = Field(..., title="To entity", description="Target entity name", alias="to") 
    relation_type: str = Field(..., title="Relation type", description="Relationship type in active voice. Example: (A) is really interested in (B)", alias="relationType")
...
class KnowledgeGraph(BaseModel):
    ...
    entities: list[Entity] = Field(default_factory=list, title="Entities", description="All entities in the knowledge graph")
    relations: list[Relation] = Field(default_factory=list, title="Relations", description="All relations between entities")


class CleanupResult(BaseModel):
    """Result of cleaning up outdated observations."""
    
    entities_processed_count: int = Field(..., title="Number of entities processed", description="Number of entities that were processed")
    observations_removed_count: int = Field(..., title="Number of observations removed", description="Total number of observations removed")
    removed_observations: list[dict] = Field(default_factory=list, title="Removed observations", description="Details of removed observations")


class DurabilityGroupedObservations(BaseModel):
    """Observations grouped by their durability type."""
    
    permanent: list[Observation] = Field(default_factory=list, title="Permanent observations", description="Observations with durability type 'permanent'")
    long_term: list[Observation] = Field(default_factory=list, title="Long-term observations", description="Observations with durability type 'long-term'")
    short_term: list[Observation] = Field(default_factory=list, title="Short-term observations", description="Observations with durability type 'short-term'")
    temporary: list[Observation] = Field(default_factory=list, title="Temporary observations", description="Observations with durability type 'temporary'")

class ObservationRequest(BaseModel):
    """Request model for managing observations for an entity in the knowledge graph. Used for both addition and deletion."""
    
    entity_name: str = Field(..., title="Entity name", description="The name of the entity to add observations to", alias="entityName")
    observations: list[Observation] = Field(..., title="Observations", description="Observations to add - objects with durability metadata", alias="observation")
    confirm: bool | None = Field(..., title="Confirm", description="Optional confirmation property. Must be passed for certain sensitive operations. ***ALWAYS VERIFY WITH THE USER BEFORE SETTING TO TRUE*** Experimental.", alias="verify")

class AddObservationResult(BaseModel):
    """Result of adding observations to an entity."""
    
    entity_name: str = Field(..., title="Entity name", description="The entity name that was updated", alias="entityName")
    added_observations: list[Observation] = Field(..., title="Added observations", description="The observations that were actually added (excluding duplicates)", alias="observation")


class DeleteObservationRequest(BaseModel):
    """Request model for deleting observations from an entity."""
    
    entity_name: str = Field(..., title="Entity name", description="The name of the entity containing the observations", alias="entityName")
    observations: list[str] = Field(..., title="Observations", description="Array of observation contents to delete", alias="observation")
